...
const { notifyRiderAssigned, notifyRiderPickedUp } = require('./telegramController');
...
const assignDelivery = async (req, res) => {
  ...
  try {
    ...
    const existing = await client.query(
      `SELECT d.id, r.name as rider_name FROM deliveries d
    ...
    if (existing.rows.length > 0) {
      ...
      return res.status(400).json({
        success: false,
        message: `This order is already assigned to ${existing.rows[0].rider_name}. Unassign them first.`,
      });
    }

    /* ── Check rider is available ── */
    const rider = await client.query('SELECT * FROM riders WHERE id = $1', [rider_id]);
    if (rider.rows.length === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ success: false, message: 'Rider not found' });
    }
    if (!rider.rows[0].is_available) {
      await client.query('ROLLBACK');
      return res.status(400).json({
        success: false,
        message: `${rider.rows[0].name} is currently busy with another delivery.`,
      });
    }
    ...
    /* Send Telegram notification to rider */
    try {
      ...
      if (orderRes.rows.length > 0) {
        await notifyRiderAssigned(rider_id, orderRes.rows[0]);
      }
    } catch (tErr) {
      console.error('Telegram notify error (non-critical):', tErr.message);
    }
  ...
  try {
    ...
    if (status === 'delivered') {
      const orderResult = await client.query(
        'SELECT payment_method, total_amount FROM orders WHERE id = $1',
        [delivery.order_id]
      );
      ...
      /* ── COD auto-confirmation ──────────────────────────────────
         For cash-on-delivery orders, the rider marking the order
         "delivered" IS their confirmation that they collected the
         cash — there's no separate office verification step. We
         trust this immediately (no blocking), but mark it with
         confirmed_by_rider = true so office staff can still spot
         and spot-check self-confirmed payments afterward without
         it holding up anything in the meantime. */
      if (order && order.payment_method === 'cod') {
        await client.query(
          ...
          [req.user?.id || null, delivery.order_id]
        );

        await client.query(
          `UPDATE cash_logs SET status = 'verified', verified_at = NOW(),
             confirmed_by_rider = true, verified_by = $1
           WHERE order_id = $2 AND status = 'pending'`,
          [req.user?.id || null, delivery.order_id]
        );
      }
    ...
    /* Send Telegram notification for picked_up */
    if (status === 'picked_up') {
      try {
        const orderRes = await pool.query(
        ...
        await notifyRiderPickedUp(delivery.rider_id, orderRes.rows[0]?.order_number);
      } catch (tErr) {
        console.error('Telegram notify error (non-critical):', tErr.message);
      }
    }
...
const resetAvailability = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `${result.rowCount} rider${result.rowCount !== 1 ? 's' : ''} reset to available`,
...
module.exports = {
  ...
  deleteRider, assignDelivery, updateDeliveryStatus, resetAvailability,
};
