...
/* ═══════════════════════════════════════════════════════════════
   ...
   for everything else. Rider's nav and behavior is unchanged from
   the original — only staff roles are new here.
═══════════════════════════════════════════════════════════════ */
const ROLE_NAV = {
  rider: {
    bottom: [
  ...
  super_admin: {
    bottom: [
      ...
      { to: '/portal/cash-admin', label: 'Cash', icon: '💰' },
    ],
    more: [
      ...
      { to: '/portal/staff', label: 'Staff', icon: '🗂️', group: 'People' },
...
/* ═══════════════════════════════════════════════════════════════
   GENERATED NAV — for cashier/dispatcher/warehouse, or any future
   role not hand-tuned above. Built from that role's permissions
   (mirrors AuthContext.js's rolePermissions) so adding a role or
   changing its permissions doesn't require editing this file.
═══════════════════════════════════════════════════════════════ */
const ROLE_PERMISSIONS = {
  cashier:    ['manage_orders', 'manage_billing', 'manage_customers', 'manage_cash'],
...
const PERMISSION_TO_SECTIONS = {
  ...
  manage_billing:   [{ to: '/portal/billing', label: 'New Bill', icon: '💵' }],
  manage_customers: [{ to: '/portal/customers', label: 'Customers', icon: '👥' }],
  manage_cash:      [{ to: '/portal/cash-admin', label: 'Cash', icon: '💰' }],
  ...
  manage_users:     [{ to: '/portal/staff', label: 'Staff', icon: '🗂️' }],
...
const PortalLayout = ({ children }) => {
  ...
  /* "View as" preview — super_admin ONLY. Any other role ignores
     this param outright; there is no way to use it to see a role
  ...
  const role = isPreviewing ? viewAsParam : (user?.role || 'rider');

  const nav = ROLE_NAV[role] || generateNavFromPermissions(role);
  const portalLabel = ROLE_PORTAL_LABEL[role] || 'Portal';

  /* Keeps ?view_as=... attached when navigating between portal
     pages during a preview. */
  const withPreview = (to) => (isPreviewing ? `${to}?view_as=${viewAsParam}` : to);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f9f8', display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', position: 'relative' }}>
      ...
      {/* Preview-mode banner — super_admin only, while viewing
          another role's portal via ?view_as= */}
      {isPreviewing && (
        <div style={{ background: '#fef3c7', borderBottom: '1px solid #fde68a', padding: '8px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#92400e' }}>
            👀 Previewing {role.replace('_', ' ')} portal
          </span>
          <button
            onClick={() => navigate('/portal')}
            style={{ background: 'none', border: 'none', color: '#92400e', fontSize: 12, fontWeight: 700, cursor: 'pointer', textDecoration: 'underline' }}
          >
            Exit preview
          </button>
        </div>
