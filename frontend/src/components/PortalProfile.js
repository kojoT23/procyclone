...
/* ═══════════════════════════════════════════════════════════════
   ...
   dispatcher / warehouse. No rider-specific fields exist for
   these accounts, so this shows their actual account info
   instead, fetched from usersAPI.getProfile() already used
...
function StaffProfile({ user }) {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      setProfile(res.data.user || res.data.profile || null);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#9ca3af', textAlign: 'center', marginTop: 16 }}>
        To change your password or account details, use the full dashboard on a desktop browser.
      </p>
    </div>
  );
}
